...
浏览器扩展（Media Control Hub）通过 Chrome Native Messaging 与桌面**中间代理层 (Proxy)** 双向通信。Proxy 随后再通过 Named Pipe 与真正的桌面 GUI 程序通讯。

- **连接机制**：浏览器 `chrome.runtime.connectNative()` 将启动 Proxy 进程产生长连接
- **Host Name**：`com.hxmonitor.proxy` (注册表指向 Proxy 的可执行文件)
- **第一段传输层 (Extension <-> Proxy)**：stdin/stdout，格式为 `[4 字节小端长度][UTF-8 JSON]`
- **第二段传输层 (Proxy <-> HxMonitor)**：Named Pipe (命名管道)
- **单条消息上限**：建议 ≤ 256KB

> **重要**：对于 Proxy 层，Host 的 stdout 只能输出协议消息。日常调试日志绝不允许污染 stdout。
...
```
Desktop                    Extension (Hub Core)           Content Scripts
   |                            |                              |
   |--- cmd/GetMediaList (id=5)->
   |                            | (若 SW 状态为空)             |
   |                            |--- broadcast queryState ---->|
   |                            |<-- stateUpdate (x N tabs) --|
   |                            | (等待 600ms)                 |
   |<-- res/GetMediaList --------|
   |   { ok, activeTabId, tabs } |
```

> **SW 重启恢复**：Service Worker 被浏览器回收后内存状态清空。收到 `GetMediaList` 时，若 `tabs` 为空则先广播 `queryState` 给所有已打开的标签页，等待 600ms 让 Content Script 回报状态，再发送完整的 `res`。
